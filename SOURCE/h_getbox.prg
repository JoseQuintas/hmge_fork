...
FUNCTION _DefineGetBox ( ControlName, ParentFormName, x, y, w, h, Value, ;
   FontName, FontSize, aToolTip, lPassword, uLostFocus, uGotFocus, uChange, right, ;
   HelpId, readonly, bold, italic, underline, strikeout, field, backcolor, fontcolor, ;
   invisible, notabstop, nId, valid, cPicture, cmessage, cvalidmessage, when, ;
   ProcedureName, ProcedureName2, abitmap, BtnWidth, lNoMinus, noborder, bInit, lAutoUpdate )
*-----------------------------------------------------------------------------*
...
*-----------------------------------------------------------------------------*
   ...
   CASE nMsg == WM_INVALID
      ...
      IF ( _IsChildOfActiveWindow( hWnd ) .OR. ;
         IsWindowHasExStyle( _HMG_aControlParenthandles [i], WS_EX_CONTROLPARENT ) ) .AND. !readonly .AND. lAllowEdit
   ...
   CASE nMsg == WM_CHAR
      ...
      CASE wParam == 26 .AND. !readonly // CTRL+Z Undo
