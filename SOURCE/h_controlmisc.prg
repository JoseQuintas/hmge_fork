...
*-----------------------------------------------------------------------------*
   ...
   CASE T == "DATEPICK"
      IF Empty ( Ts := _SetGetDatePickerDateFormat ( _HMG_aControlNames [ix] , GetParentFormName( ix ) ) ) .OR. ;
         !( "HH:" $ Upper( Ts ) ) .OR. !( "mm:" $ Lower( Ts ) )
...
*-----------------------------------------------------------------------------*
   ...
   ELSEIF !( "LABEL" $ T .OR. T == "HYPERLINK" ) .AND. T != "RICHEDIT" .AND. T != "TREE"
      ...
      IF T == "GRID" .AND. ISARRAY ( xPreviousValue )
      ...
      ELSE
         ...
         IF ISARRAY ( Value )
            DO CASE
               ...
               OTHERWISE
                  IF _HMG_aControlMiscData1 [ix] [ 1 ] <= 0
                     _HMG_aControlMiscData1 [ix] [ 1 ] := 1
                  ELSEIF _HMG_aControlMiscData1 [ix] [ 1 ] > ListViewGetItemCount ( c )
                    _HMG_aControlMiscData1 [ix] [ 1 ] := ListViewGetItemCount ( c )
                  ENDIF
                  IF _HMG_aControlMiscData1 [ix] [ 17 ] <= 0
                     _HMG_aControlMiscData1 [ix] [ 17 ] := 1
                  ELSEIF _HMG_aControlMiscData1 [ix] [ 17 ] > ListView_GetColumnCount ( c )
                    _HMG_aControlMiscData1 [ix] [ 17 ] := ListView_GetColumnCount ( c )
                  ENDIF
            ENDCASE
         ENDIF
#endif
   ...
   CASE T == "TAB"
      ...
      IF Len ( _HMG_aControlPageMap [ix] ) > 0
         nOldPage := _GetValue ( , , ix )
         TabCtrl_SetCurSel ( c , nValue )
         _SetFocus ( , , ix )
         IF ! _HMG_ProgrammaticChange
            UpdateTab ( ix )
            _DoControlEventProcedure ( _HMG_aControlChangeProcedure [ix] , ix , 'CONTROL_ONCHANGE' )
         ELSEIF ! ISBLOCK( _HMG_aControlChangeProcedure [ix] ) .OR. ;
            _DoControlEventProcedure ( _HMG_aControlChangeProcedure [ix] , ix , 'CONTROL_ONCHANGE' , nOldPage )
            UpdateTab ( ix )
         ELSE
            TabCtrl_SetCurSel ( c , nOldPage )
         ENDIF
      ENDIF
   ...
   CASE T == "DATEPICK"
      IF Empty ( Value )
         SetDatePickNull ( c )
      ELSE
         IF HB_ISDATETIME ( value ) .OR. ISARRAY ( value )
            IF ISARRAY ( value )
               IF Len ( value ) >= 7
...
*-----------------------------------------------------------------------------*
   ...
   CASE "LIST" $ T
      ...
      ELSE
         IF _HMG_aControlMiscData1 [ix] [2] .AND. ISARRAY( value )
   ...
   CASE "GRID" $ T
      IF _HMG_aControlMiscData1 [ix] [5] == .F.
         IF ISNUMERIC ( Parent )
            _AddGridRow ( ControlName, ParentForm, value, Parent )
         ELSE
   ...
   CASE "GRID" $ T
      IF _HMG_aControlMiscData1 [ix] [5] == .F.
         ...
         IF _HMG_aControlFontColor [ix] == .T. .AND. T == "GRID"
            _HMG_aControlMiscData1 [ix] [ 1 ] := LISTVIEW_GETFOCUSEDITEM ( c )
         ENDIF
...
*-----------------------------------------------------------------------------*
   ...
   CASE T == "CHARMASKTEXT"
      ...
      x := _HMG_aControlInputMask [i]
      FOR EACH t IN x

         IF hmg_IsDigit( t ) .OR. hmg_IsAlpha( t ) .OR. t == '!'
            MaskStart := hb_enumindex( t )
            EXIT
         ENDIF
...
*-----------------------------------------------------------------------------*
   ...
   CASE T == "CHECKLABEL"
      DisableWindow ( c )
      IF ISARRAY( _HMG_aControlPicture [y] )  // get CheckLabel bitmap
         IF _HMG_aControlPicture [y] [1] == "MINIGUI_SWITCH_ON"
            ReplaceCheckImage ( c, 'MINIGUI_SWITCH_ON_GRAY', 'MINIGUI_SWITCH_GRAY', _HMG_aControlRangeMin [y], GetChkLabel ( c ), _HMG_aControlRangeMax [y] )
         ENDIF
      ENDIF
   ...
   CASE t == "GETBOX"
      FOR EACH w IN _HMG_aControlRangeMin [y]
         DisableWindow ( w )
      NEXT

   CASE T == "BTNTEXT" .OR. T == "BTNNUMTEXT"
      FOR EACH w IN _HMG_aControlSpacing [y]
         DisableWindow ( w )
      NEXT
...
*-----------------------------------------------------------------------------*
   ...
   CASE T == "CHECKLABEL"
      ...
      IF ISARRAY( _HMG_aControlPicture [y] )  // get CheckLabel bitmap
         IF _HMG_aControlPicture [y] [1] == "MINIGUI_SWITCH_ON"
            ReplaceCheckImage ( c, 'MINIGUI_SWITCH_ON', 'MINIGUI_SWITCH_OFF', _HMG_aControlRangeMin [y], GetChkLabel ( c ), _HMG_aControlRangeMax [y] )
         ENDIF
      ENDIF
...
*-----------------------------------------------------------------------------*
   ...
   CASE t == 'TREE'
      ...
      TreeView_SetItem ( c , ItemHandle , Value )
...
*-----------------------------------------------------------------------------*
   ...
   CASE t == "TREE"
      ...
      RetVal := TreeView_GetItem ( c , ItemHandle )
...
*-----------------------------------------------------------------------------*
   ...
   OTHERWISE
      ...
      IF T == "COMBO"
         xx := iif( _HMG_aControlMiscData1 [x] [1] == 0, 11, 3 )
         IF _HMG_aControlMiscData1 [x] [xx] != NIL
            ComboSetItemHeight ( c , _HMG_aControlMiscData1 [x] [xx] )
         ENDIF
      ENDIF
   ...
   OTHERWISE  // picture for [check]buttons
      ...
      IF _HMG_aControlEnabled [i]
         ...
         IF ! Empty ( _HMG_aControlBrushHandle [i] )
            ...
            ELSE
               IF ! Empty ( _HMG_aControlMiscData1 [i] ) .AND. IsHIcon( _HMG_aControlMiscData1 [i] )
         ...
         IF _HMG_aControlMiscData1 [i] == 0  // bitmap
            ...
            ELSE
               IF t == "OBUTTON"
                  IF ! IsWindowSized ( _HMG_aControlParentHandles [i] )
                     w := BmpSize( cImage )
                     IF w [1] > 0 .AND. w [2] > 0
                        _HMG_aControlHeadClick [i][1] := w [1]
                        _HMG_aControlHeadClick [i][2] := w [2]
                     ENDIF
                  ENDIF
...
*-----------------------------------------------------------------------------*
STATIC PROCEDURE TreeItemChangeImage ( ControlName, ParentForm, nItem, aImage )
*-----------------------------------------------------------------------------*
...
*-----------------------------------------------------------------------------*
   ...
   CASE t == "MESSAGEBAR"
      IF _IsControlDefined ( 'StatusBarKbd' , ParentForm )
         x := GetControlIndex ( 'StatusBarKbd' , ParentForm )
         ReleaseControl ( _HMG_aControlHandles [x] )
         _EraseControl ( x , k )
      ENDIF

      IF _IsControlDefined ( 'StatusTimer' , ParentForm )
         x := GetControlIndex ( 'StatusTimer' , ParentForm )
         ReleaseControl ( _HMG_aControlHandles [x] )
         _EraseControl ( x , k )
      ENDIF

      IF _IsControlDefined ( 'StatusKeyBrd' , ParentForm )
         x := GetControlIndex ( 'StatusKeyBrd' , ParentForm )
         ReleaseControl ( _HMG_aControlHandles [x] )
         _EraseControl ( x , k )
      ENDIF
         ...
         FOR w := 1 TO Len ( _HMG_aControlPageMap [i] [r] )
            ...
            ELSE
               ...
               FOR x := 1 TO Len ( _HMG_aControlHandles )
                  IF ISARRAY( _HMG_aControlHandles [x] )
...
*-----------------------------------------------------------------------------*
   ...
   IF _HMG_aControlType [i] $ "OBUTTON" .AND. ;
      ! Empty( _HMG_aControlMiscData1 [i] ) .AND. IsHIcon( _HMG_aControlBrushHandle [i] )
      DestroyIcon ( _HMG_aControlBrushHandle [i] )
   ELSE
      DeleteObject ( _HMG_aControlBrushHandle [i] )
   ENDIF
   ...
   CASE t == 'COMBO'
      IF _IsComboExtend ( _HMG_aControlNames [i] , _HMG_aFormNames [p] )
         ImageList_Destroy ( _HMG_aControlBrushHandle [i] )
      ENDIF
   ...
   CASE t == 'IMAGELIST'
      ImageList_Destroy ( _HMG_aControlHandles [i] )
   ...
   CASE 4 // PCount() == 4 (CONTROL)
      ...
      CASE Arg3 == "ITEMHEIGHT"
         ...
         IF GetControlType( Arg2, Arg1 ) == "COMBO"
            ...
            _HMG_aControlMiscData1 [GetControlIndex ( Arg2 , Arg1 )] [iif ( _IsComboExtend ( Arg2 , Arg1 ), 3, 11)] := Arg4
         ELSE
   ...
   CASE 5 // PCount() == 5 (CONTROL WITH ARGUMENT OR TOOLBAR BUTTON OR SPLITBOX CHILD CONTROL WITHOUT ARGUMENT)
      ...
      ELSE
         ...
         CASE Arg3 == "HEADERDYNAMICFORECOLOR"
            ...
            _SetGetGridProperty ( Arg2 , Arg1 , _GRID_COLUMN_HEADERDYNAMICFORECOLOR_ , Arg4 , Arg5 )

         CASE Arg3 == "HEADERDYNAMICBACKCOLOR"

            _SetGetGridProperty ( Arg2 , Arg1 , _GRID_COLUMN_HEADERDYNAMICBACKCOLOR_ , Arg4 , Arg5 )

         CASE Arg3 == "HEADERDYNAMICFONT"

            _SetGetGridProperty ( Arg2 , Arg1 , _GRID_COLUMN_HEADERDYNAMICFONT_ , Arg4 , Arg5 )
...
*-----------------------------------------------------------------------------*
   ...
   CASE 2 // PCount() == 2 (WINDOW)
      ...
      CASE Arg2 == "PARENT"  // GF 02/01/23

#ifdef _HMG_COMPAT_
         IF GetFormNameByHandle ( GetParent ( GetFormHandle( Arg1 ) ) , @xData ) > 0
            RetVal := xData
         ENDIF
#else
         IF ( ix := AScan ( _HMG_aFormHandles , GetParent ( GetFormHandle( Arg1 ) ) ) ) > 0
            RetVal := _HMG_aFormNames [ix]
         ENDIF
#endif
      CASE Arg2 == "GRIPPERTEXT"
      ...
      CASE Arg2 == "HEIGHT"
         ...
         RetVal := _GetWindowSizePos ( Arg1 ) [4]

      CASE Arg2 == "WIDTH"

         RetVal := _GetWindowSizePos ( Arg1 ) [3]

      CASE Arg2 == "COL"

         RetVal := _GetWindowSizePos ( Arg1 ) [2]
   ...
   CASE 3 // PCount() == 3 (CONTROL)
      ...
      CASE Arg3 == "IMAGELIST"
         ...
         RetVal := TreeView_GetImageList ( GetControlHandle(Arg2,Arg1) , TVSIL_NORMAL ) 

      CASE Arg3 == "HASLINES"

         RetVal := HMG_IsWindowStyle ( GetControlHandle( Arg2, Arg1 ), TVS_HASLINES, .F. )

      CASE Arg3 == "FULLROWSELECT"

         RetVal := HMG_IsWindowStyle ( GetControlHandle( Arg2, Arg1 ), TVS_FULLROWSELECT, .F. )
#endif
      CASE Arg3 == "VALIDMESSAGE" .OR. Arg3 == "EDITABLE"
   ...
   CASE 4 // PCount() == 4 (CONTROL WITH ARGUMENT OR TOOLBAR BUTTON OR (JK) HMG 1.0 Experimental Buid 6 GRID/BROWSE COLUMN - ColumnWidth OR SPLITBOX CHILD WITHOUT ARGUMENT)
      ...
      ELSE
         ...
         CASE Arg3 == "HEADERDYNAMICFORECOLOR"
            ...
            RetVal := _SetGetGridProperty ( Arg2 , Arg1 , _GRID_COLUMN_HEADERDYNAMICFORECOLOR_ , Arg4 )

         CASE Arg3 == "HEADERDYNAMICBACKCOLOR"

            RetVal := _SetGetGridProperty ( Arg2 , Arg1 , _GRID_COLUMN_HEADERDYNAMICBACKCOLOR_ , Arg4 )

         CASE Arg3 == "HEADERDYNAMICFONT"

            RetVal := _SetGetGridProperty ( Arg2 , Arg1 , _GRID_COLUMN_HEADERDYNAMICFONT_ , Arg4 )
         ...
         CASE Arg3 == "IMAGEINDEX"
            ...
            RetVal := TreeItemGetImageIndex ( Arg2 , Arg1 , Arg4 )

         CASE Arg3 == "HASBUTTON"

            RetVal := TreeView_GetHasButton ( GetControlHandle( Arg2, Arg1 ), TreeItemGetHandle ( Arg2, Arg1, Arg4 ) )
#endif
...
*-----------------------------------------------------------------------------*
   ...
   IF PCount() == 2 // Window
      ...
      CASE Arg2 == 'ENABLEUPDATE'

         LockWindowUpdate ( 0 )
         RedrawWindow ( GetFormHandle( Arg1 ) )

      CASE Arg2 == 'DISABLEUPDATE'

         LockWindowUpdate ( GetFormHandle( Arg1 ) )
   ...
   ELSEIF PCount() == 3 // CONTROL
      ...
      CASE Arg3 == 'GETARRAY' // GF

         RETURN _GetControlArray ( Arg2 , Arg1 )
   ...
   ELSEIF PCount() == 4   // CONTROL WITH 1 ARGUMENT OR SPLITBOX CHILD WITHOUT ARGUMENT
      ...
      CASE Arg3 == 'SETHEADERIMAGES' // GF

         _SetGridHeaderImages ( Arg2 , Arg1 , Arg4 )
   ...
   ELSEIF PCount() == 5 .AND. ISCHARACTER ( Arg3 ) // CONTROL WITH 2 ARGUMENTS OR SPLITBOX CHILD WITH 1 ARGUMENT
   ...
   ELSEIF PCount() == 5  .AND. ISNUMERIC ( Arg3 ) // TAB CHILD WITHOUT ARGUMENTS

      IsControlInTabPage ( Arg1 , Arg2 , Arg3 , Arg4 )
      DoMethod ( Arg1 , Arg4 , Arg5 )

   ELSEIF PCount() == 6 .AND. ISCHARACTER ( Arg3 ) // CONTROL WITH 3 ARGUMENTS OR SPLITBOX CHILD WITH 2 ARGUMENTS
   ...
   ELSEIF PCount() == 6 .AND. ISNUMERIC ( Arg3 ) // TAB CHILD WITH 1 ARGUMENT

      IsControlInTabPage ( Arg1 , Arg2 , Arg3 , Arg4 )
      DoMethod ( Arg1 , Arg4 , Arg5 , Arg6 )

   ELSEIF PCount() == 7 .AND. ISCHARACTER ( Arg3 ) // CONTROL WITH 4 ARGUMENTS OR SPLITBOX CHILD WITH 3 ARGUMENTS
   ...
   ELSEIF PCount() == 7 .AND. ISNUMERIC ( Arg3 ) // TAB CHILD WITH 2 ARGUMENTS
   ...
   IF ( niControl := GetControlIndex ( cControlName , cParentWindowName ) ) > 0
      ...
      FOR EACH tabpage IN _HMG_aControlPageMap [ niTab ]
         ...
         FOR EACH c IN tabpage
            ...
            IF ISNUMERIC ( c ) .AND. ISNUMERIC ( xControlHandle )
               IF c == xControlHandle
                  nRetVal := k
                  EXIT
               ENDIF

            ELSEIF ISARRAY ( c ) .AND. ISARRAY ( xControlHandle )
            ...
            ELSEIF ISARRAY ( c ) .AND. ISNUMERIC ( xControlHandle )
...
*-----------------------------------------------------------------------------*
   ...
   IF ( i := GetControlIndex ( cControlName, cWindowName ) ) > 0
      ...
      IF !Empty ( _HMG_SplitLastControl ) .AND. _HMG_aControlRow [i] == NIL .AND. _HMG_aControlCol [i] == NIL ;
         .OR. ;
...
*-----------------------------------------------------------------------------*
STATIC PROCEDURE _SetArrayToControl ( ControlName , ParentForm , aValue )  // GF 03/30/16
*-----------------------------------------------------------------------------*
   LOCAL t , i , BackValue
#ifdef _TSBROWSE_
   LOCAL oGet
...
*-----------------------------------------------------------------------------*
STATIC PROCEDURE _SetGridHeaderImages ( ControlName, ParentForm, aHeaderImages )
*-----------------------------------------------------------------------------*
   LOCAL i
   LOCAL hHeaderImageList

   IF ( i := GetControlIndex ( ControlName, ParentForm ) ) > 0

      _HMG_aControlPicture[ i ] := aHeaderImages

      IMAGELIST_DESTROY ( _HMG_aControlRangeMin[ i ] )

      hHeaderImageList := AddListViewBitmapHeader ( _HMG_aControlHandles[ i ], aHeaderImages )

      _HMG_aControlRangeMin[ i ] := hHeaderImageList
...
*-----------------------------------------------------------------------------*
   ...
   hb_default( @OnActionCodeBlock, {|| NIL } )
   hb_default( @lCheckDown, .T. )
   hb_default( @lCheckMatchCase, .F. )
   hb_default( @lCheckWholeWord, .F. )
...
*-----------------------------------------------------------------------------*
   ...
   CASE T == 'COMBO'
      IF ISCHARACTER ( _HMG_aControlSpacing [i] )
         _DataComboRefresh ( i )
         ...
         IF _HMG_aControlParentHandles [i] == hFormHandle
            ...
            IF ISNUMERIC ( hControl )
               ...
               IF hControl == hFocusedControlHandle .OR. ;
                  ( ! Empty( _HMG_aControlType [i] ) .AND. ( ISNUMERIC( _HMG_aControlRangeMin [i] ) .AND. _HMG_aControlRangeMin [i] == hFocusedControlHandle ) .OR. ;
                                                           ( ISNUMERIC( _HMG_aControlRangeMax [i] ) .AND. _HMG_aControlRangeMax [i] == hFocusedControlHandle ) ) .OR. ;
                  ( ISARRAY( _HMG_aControlSpacing [i] ) .AND. ISNUMERIC( _HMG_aControlSpacing [i][1] ) .AND. AScan ( _HMG_aControlSpacing [i], hFocusedControlHandle ) > 0 ) .OR. ;
                  ( ISARRAY( _HMG_aControlRangeMin [i] ) .AND. ISNUMERIC( _HMG_aControlRangeMin [i][1] ) .AND. AScan ( _HMG_aControlRangeMin [i], hFocusedControlHandle ) > 0 )
   ...
   IF ( i := GetControlIndex ( ControlName, ParentForm ) ) > 0
      IF ISARRAY ( aLimits )
         IF Len ( aLimits ) == ListView_GetColumnCount ( _HMG_aControlHandles [i] )
            FOR EACH z IN aLimits
               IF ! ISARRAY ( z ) .OR. ;
                  ! ISNUMERIC ( z[1] ) .OR. ! ISNUMERIC ( z[2] )
                  lError := .T.
                  EXIT
               ENDIF
            NEXT
...
*-----------------------------------------------------------------------------*
   ...
   IF ( i := GetControlIndex ( ControlName , ParentForm ) ) > 0 .AND. ISARRAY ( aOptions )
      ...
      nOptions := Len ( _HMG_aControlHandles [i] )
...
*-----------------------------------------------------------------------------*
PROCEDURE _SetStatusIcon ( ControlName , ParentForm , Item , Icon , hIcon )
*-----------------------------------------------------------------------------*
   LOCAL nItem As Numeric
   LOCAL i
   ...
   IF ( i := GetControlIndex ( ControlName , ParentForm ) ) > 0
      ...
      ELSE
         SetStatusItemIcon ( _HMG_aControlHandles [i] , nItem , , hIcon )
      ENDIF
   ENDIF
...
*-----------------------------------------------------------------------------*
   ...
   FOR EACH h IN _HMG_aControlParentHandles
      ...
      IF h == hWnd .AND. iif( cUserType == "ALL", .T., GetUserControlType ( _HMG_aControlNames [i], GetParentFormName( i ) ) == cUserType )
         ...
         IF ISNUMERIC ( _HMG_aControlHandles [i] )
         ...
         ELSEIF ISARRAY ( _HMG_aControlHandles [i] )
...
*-----------------------------------------------------------------------------*
   ...
   IF i > 0 .AND. _HMG_aControlType [i] == 'TAB' .AND. _HMG_aControlMiscData1 [i] [5]  // HotTrack
...
*-----------------------------------------------------------------------------*
   ...
   IF i > 0 .AND. ( _HMG_aControlType [i] == "GETBOX" .OR. "GRID" $ _HMG_aControlType [i] )
