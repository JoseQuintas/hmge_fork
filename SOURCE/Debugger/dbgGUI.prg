...
#define _HMG_DEBUGGER_ICON_       32001
/*****************************************************************************************************/
...
STATIC FUNCTION LoadPrg( cGrid_SourceCode, cFileName )
   ...
   FOR EACH cLine IN aLineCode
      ...
      ENDIF
      IF HMG_IsCurrentCodePageUnicode() == .T. .AND. hb_StrIsUTF8( aLineCode [i] ) == .F.
         aLineCode [i] := hb_Translate( aLineCode [i], NIL, "UTF8" )
      ELSEIF HMG_IsCurrentCodePageUnicode() == .F. .AND. hb_StrIsUTF8( aLineCode [i] ) == .T.
         aLineCode [i] := hb_UTF8ToStr( aLineCode [i], NIL )
      ENDIF
      DoMethod( "_HMG_FormDebugger", cGrid_SourceCode, "AddItem", { Str( i, 7, 0 ), RTrim( cLine ) } )
   NEXT
...
STATIC FUNCTION HMG_IsCurrentCodePageUnicode()
