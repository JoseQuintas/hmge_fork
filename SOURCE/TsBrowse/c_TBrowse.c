...
{
   HWND        hWnd;
   DWORD       dwExStyle = hmg_par_DWORD( 1 );
#ifdef UNICODE
   void        *hClassName;
   ...
   HWND        hWndParent = hmg_par_raw_HWND( 9 );
   HMENU       hMenu = ( HMENU ) NULL;
   HINSTANCE   hInstance = hmg_par_raw_HINSTANCE( 11 );

   hWnd = CreateWindowEx( dwExStyle, cClass, cTitle, dwStyle, x, y, nWidth, nHeight, hWndParent, hMenu, hInstance, NULL );
...
HB_FUNC( TSDRAWCELL )
{
   ...
   HFONT    hFont = hmg_par_raw_HFONT( 10 );
   HBITMAP  hBitMap = hmg_par_raw_HBITMAP( 11 );
...
HB_FUNC( GETSCRLRANGE ) // ( hWnd, nFlags )
{
   ...
   GetScrollRange( hmg_par_raw_HWND( 1 ), // its hWnd
...
HB_FUNC( INITEDSPINNER )
{
   ...
   HWND                 hedit;
   HWND                 hupdown;
   DWORD                dwStyle = WS_CHILD | WS_VISIBLE | UDS_ARROWKEYS | UDS_ALIGNRIGHT | UDS_SETBUDDYINT | UDS_NOTHOUSANDS | UDS_HOTTRACK;
   int                  iMin, iMax;
   ...
   hedit = hmg_par_raw_HWND( 2 );

   iMin = hb_parni( 7 );
   iMax = hb_parni( 8 );

   hupdown = CreateUpDownControl( dwStyle, hb_parni( 3 ), hb_parni( 4 ), hb_parni( 5 ), hb_parni( 6 ), hwnd, 0, GetModuleHandle( NULL ), hedit, iMax, iMin, hb_parni( 9 ) );

   SendMessage( hupdown, UDM_SETRANGE32, ( WPARAM ) iMin, ( LPARAM ) iMax );

   hmg_ret_raw_HWND( hupdown );
}
...
HB_FUNC( SETINCREMENTSPINNER )
{
   ...
   SendMessage( hmg_par_raw_HWND( 1 ), UDM_GETACCEL, ( WPARAM ) 1, ( LPARAM ) & inc );
