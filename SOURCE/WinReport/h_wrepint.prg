        ...
        do case
           ...
           case ascan(arryPar,"SET")=1
                do case
                   ...
                   case ascan(ArryPar,[SAVEBUTTONS])=2 .and. len(arrypar)= 3
                        hbprn:Savebuttons := if(eval(chblk,arrypar,[SAVEBUTTONS])=[OFF],.F.,.T.)
...
*-----------------------------------------------------------------------------*
 ...
 _definefont("XTX", argf1 , argsize )
 _arg5:= ( Getfontheight( "X" ) * .355 ) / 2
 RELEASE FONT "XTX"
 *_arg5:= argSize*25.4/100
 if empty(onlyone)
    _HMG_PRINTER_H_PRINT ( if(MGSYS,_HMG_SYSDATA [ 374 ],_hmg_printer_hdc) ;
   , arg1-oWr:aStat[ 'Hbcompatible' ]-_arg5 +gap ;
   , arg2  ;
   , argf1 ;
   , argsize ;
   , argcolor1[1] ;
   ...
   oWr:aStat [ 'Yes_Memo' ] :=.t.
     ...
     for mcl=2 to len(arrymemo)
         ...
         Endif
         _HMG_PRINTER_H_PRINT ( if(MGSYS,_HMG_SYSDATA [ 374 ],_hmg_printer_hdc) ;
         , (nline*lstep)-oWr:aStat[ 'Hbcompatible' ]-_arg5 + gap , arg2, argf1 , argsize , argcolor1[1], argcolor1[2], argcolor1[3] ;
