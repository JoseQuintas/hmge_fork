...
HB_FUNC( INITBTNTEXTBOX )
{
   ...
   DWORD iStyle;                 // TEXTBOX window base style.
...
HB_FUNC( REDEFBTNTEXTBOX )
{
   ...
   if( hb_parc( 4 ) != NULL )
   {
      ...
      if( himage2 != NULL )
      {
         ...
         {
            DeleteObject( himage2 );
            himage2 = ( HWND ) LoadImage( GetResources(), lpImageName2, IMAGE_BITMAP, BtnWidth2 - 4, height - 6, LR_LOADMAP3DCOLORS | LR_LOADTRANSPARENT );
...
LRESULT CALLBACK OwnBtnTextProc( HWND hwnd, UINT Msg, WPARAM wParam, LPARAM lParam )
{
   ...
   OldWndProc = ( WNDPROC ) ( HB_PTRUINT ) GetProp( hwnd, TEXT( "oldbtntextproc" ) );

   switch( Msg )
   {
      case WM_DESTROY:
         SubclassWindow2( hwnd, OldWndProc );
         RemoveProp( hwnd, TEXT( "oldbtntextproc" ) );
         break;
