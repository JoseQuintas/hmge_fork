   ...
   if( pArea && ! pArea->fReadonly && ! pArea->fShared )
   {
      ...
      else
      {
         ...
         if( bOk )
         {
            ...
            (
               hFile,
               ( HB_FOFFSET ) pArea->uiHeaderLen + ( HB_FOFFSET ) pArea->uiRecordLen * ( HB_FOFFSET ) ( pArea->ulRecCount - ulCount ),
               FS_SET
            );
            ...
            hb_fsSeekLarge( hFile, ( HB_FOFFSET ) pArea->uiHeaderLen + ( HB_FOFFSET ) pArea->uiRecordLen * ( HB_FOFFSET ) ( ulRec + ulCount - 1 ), FS_SET );
