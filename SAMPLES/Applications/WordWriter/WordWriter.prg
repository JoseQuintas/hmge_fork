...
LOCAL aViewRect     := {}
LOCAL cTempFile     := GETTEMPFOLDER() + hb_ps() + '_richedit.rtf'
...
BEGIN SEQUENCE
  ...
  IF !lInit
    IF lSetWordWrap == lWordWrap
      BREAK
    END
    wMain.ebDoc.SAVEFILE(cTempFile, .N., RICHEDITFILEEX_RTF)
    nCaretPos := wMain.ebDoc.CARETPOS
    wMain.ebDoc.RELEASE 
    lWordWrap := lSetWordWrap
  END

  DEFINE RICHEDITBOXEX ebDoc
    PARENT    wMain
    ROW       143
    COL       0
    ...
    MAXLENGTH -1
    HSCROLL   lWordWrap
    ONCHANGE  iif(lInit, , EditKey())
    ONSELECT  Refresh()
    ONLINK    DoLink()
    ONVSCROLL wMain.ebDoc.REDRAW
  END RICHEDITBOXEX
  DocSize()

  IF !lInit
    wMain.ebDoc.LOADFILE(cTempFile, .N., RICHEDITFILEEX_RTF)
  ...
  ClearUndoBuffer()
  SET CONTEXT MENU CONTROL ebDoc OF wMain OFF
...
LOCAL cKey := Right( wMain.ebDoc.VALUE, 1 ) //HMG_GETLASTCHARACTEREX()
...
LOCAL aMainRowCol := ;
  GetPos_ClientToScreen( GetFormHandle('wMain'), ;
...
DEFINE WINDOW wFileMenu ;
  ...
  @  10, 465 BUTTON btCancel ;
             CAPTION 'x' BOLD FLAT ; // U+00D7 MULTIPLICATION SIGN
...
END
BEGIN SEQUENCE
  wMain.ebDoc.LOADFILE(cFileName, .N., nFormat)
...
LOCAL lUnicode := RichEditBox_HasNonAsciiChars(wMain.ebDoc.HANDLE)
LOCAL cNewFileName
...
BEGIN SEQUENCE
  ...
  CASE 5
    ...
    EXIT
  END
  wMain.ebDoc.SAVEFILE(cFileName, .N., nFormat)
  lModified := .N.
RECOVER
...
LOCAL aListRowCol := ;
  GetPos_ClientToScreen( GetControlHandle('taTop', 'wMain'), ;
...
PROCEDURE DrawBorder( name, width, height )
