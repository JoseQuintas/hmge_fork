 ...
 *
 * Copyright 2004-2021 Grigory Filatov <gfilatov@inbox.ru>
 */
...
#define VERSION ' 0.80'
#define COPYRIGHT ' 2004-2021 Grigory Filatov'
...
*--------------------------------------------------------*
   ...
   LOCAL cFont := _GetSysFont(), nSize := 10, lBold := .F., lItalic := .F., aFntClr := {0,0,0}
...
*--------------------------------------------------------*
   ...
   IF !Empty(nFocus)
	cSaveFile := PutFile( { {"DBF files (*.dbf)", "*.dbf"}, {"Text files (*.txt)", "*.txt"}, ;
		{"Data files (*.dat)", "*.dat"}, {"Excel files (*.xlsx)", "*.xlsx"}, ;
