...
FUNCTION myTsbInit( ob, oTsb )  // настройки
   ...
   WITH OBJECT ob
      ...
      //:nHeightCell += 6            // добавка к высоте
...
////////////////////////////////////////////////////////////////////////
FUNCTION myTsbColor( oBrw )
   LOCAL nCol, oCol, O, aBClrBr, nBClrCell, nBClr2Prc, nGrad, nBClrHF, nBClrSH

   aBClrBr   := { 232,212,244 }                // цвет фона под таблицей
   nBClrCell := RGB(232,212,244)               // цвет фона таблицы
   nBClr2Prc := RGB(238,130,238)               // цвет фона таблицы %2
   nBClrHF   := RGB(139,  0,139)               // цвет фона шапки и подвала таблицы
   nBClrSH   := RGB(138, 43,226)               // цвет фона суперхидера
   ...
   WITH OBJECT oBrw:Cargo
      ...
      :nClrBC     := nBClrCell                          // цвет фона таблицы
      :nClrPrc    := nBClr2Prc                          // строка % 2
      ...
      :nHead2     := nBClrHF                            // подвал и шапка таблицы
      :nHead1     := RGB(164,164,164)                   // подвал и шапка таблицы
      ...
      :nHeadSpH   := RGB(48,29,26)                      // градиент спецхидер
      :nClr16     := {:nHeadSpH,nBClrSH}                // 16, фона спецхидер
      :nClr17     := CLR_YELLOW                         // 17, текста спецхидер
      :nClr16All  := {:nHead1,CLR_HRED}                 // 16, фона спецхидер все записи
      ...
      :aClrBrush  := aBClrBr                            // цвет фона под таблицей - резерв
  END WITH

   WITH OBJECT oBrw
      O := :Cargo
      :nClrLine := RGB(186,85,211)                               // COLOR_GRID
      ...
      :SetColor( {  6 }, { { |a,b,c| a:=nil, iif( c:nCell == b, -CLR_HRED, -RGB(1,1,1) ) } } )  // 6 , фона курсора
      :SetColor( {  7 }, { { || CLR_ORANGE              } } )    // 7 , текста редактируемого поля
      :SetColor( {  8 }, { { || CLR_GREEN               } } )    // 8 , фона редактируемого поля
      :SetColor( {  9 }, { { || CLR_WHITE               } } )    // 9 , текста подвала таблицы
      :SetColor( { 10 }, { { || { O:nHead2, O:nHead1 }  } } )    // 10, фона подвала таблицы
      ...
      //:hBrush  := CreateSolidBrush(o:aClrBrush[1], o:aClrBrush[2], o:aClrBrush[3])  // цвет фона под таблицей - не надо
   END WITH
...
FUNCTION myTsbSuperHd( oBrw, oTsb )
   ...
   WITH OBJECT oBrw
      ...
      :nHeightSuper := nHFont * 3 //+ nHFont/2   // 3 строки
...
FUNCTION myTsbEdit( oBrw, oTsb )
   ...
   aColPrc := oTsb:aColPrc       // тип обработки колонки
   aFunc1  := oTsb:aFunc1        // функция-1 :bPrevEdit для обработки колонки таблицы
   aFunc2  := oTsb:aFunc2        // функция-2 :bPostEdit для обработки колонки таблицы
   aDim    := oTsb:aTable        // весь массив таблицы в cargo окна
   ...
   FOR EACH oCol IN oBrw:aColumns
      // i := hb_EnumIndex(oCol)
      // ? i, oCol:cName, oCol:cField, oCol:cFieldTyp
      IF oCol:cFieldTyp $ "+=@T"
         oCol:lEdit := .F.
      ENDIF
   NEXT

   ? "======= " + ProcName() , "SELECTOR .OR. ORDKEYNO = ", nJ, "nO=",nO, nS, "nP=", nP
   FOR EACH oCol IN oBrw:aColumns
       i := hb_EnumIndex(oCol)
       ...
       ELSE
          ...
          oCol:bPrevEdit := {|xv,ob|
                              ...
                              Local aVal, cMsg, nTm, lWrtUDT
                              Local nOrder := INDEXORD()
                              nTm := VAL( SUBSTR(TIME(),1,2)+SUBSTR(TIME(),4,2) )
                              nc  := ob:nCell
                              oc  := ob:GetColumn( nc )
                              cNm := oc:cName
                              xv  := oc:Cargo
                              ...
                              lWrtUDT:= .F.
                              IF (ob:cAlias)->(RLock())               // делать самому
                                 ob:Cargo:nModify ++  // была модификация таблицы
                                 IF cPrcs = "M"
                                    xGet := CellEditMemo(xGet, ob)
                                    IF LEN(xGet) > 0
                                       ob:SetValue(cNm , xGet )
                                       lWrtUDT := .T.
                                    ENDIF
                                    lRet := .F.                    // не попадать в станд.getbox
                                    DO EVENTS
                                 ELSEIF cPrcs $ "CNDL"
                                    IF cPrcs == "C" .AND. AT(CRLF,xGet) > 0
                                       xGet := CellEditMemo(xGet, ob)
                                       IF LEN(xGet) > 0
                                          ob:SetValue(cNm , xGet )
                                          lWrtUDT := .T.
                                    ...
                                    ELSE
                                       ob:SetValue(cNm , xGet )
                                       lRet := .T.                    // попадать в станд.getbox
                                    ENDIF
                                    lWrtUDT := .T.
                                    DO EVENTS
                                 ELSEIF cPrcs $ "STKB"
                              ...
                              IF lWrtUDT
                                 ...
                                 (ob:cAlias)->TIMEVVOD  := nTm        // время правки
                              ENDIF
                              (ob:cAlias)->(dbUnLock())
                              ob:Skip(0)
        ...
        oCol:bPostEdit := {|uVal,ob|
                              ...
                              Local aDimTsb     := o:aTable       // весь массив таблицы
                              Local aVal, cStr, lSay, xv, nOrder := INDEXORD()
                              nc  := ob:nCell
                              oc  := ob:GetColumn(nc)
                              xv  := oc:Cargo
                              cNm := oc:cName
                              ...
                              ? ":bPostEdit  , xGet=", xGet, "uVal=", uVal,"INDEXORD()=",nOrder
                              ////////////// обработка колонок по типам
                              cStr  := HB_ValToExp(aDimTsb[ni])
                              aVal  := { cNm, cPrcs, ni, aDimTsb[ni] }  // передать в функции
                              cPrcs := aColProcess[ni]                  // тип обработки колонки
...
FUNCTION myTsb_Before( oBrw )
   ...
   WITH OBJECT oBrw
      ...
      */
      ...
      :UserKeys(VK_INSERT, {|ob| DoEvents(), _wPost("_RecIns", ob:cParentWnd, "_RecIns"), .F. })
      :UserKeys(VK_DELETE, {|ob| DoEvents(), _wPost("_RecDel", ob:cParentWnd, "_RecDel"), .F. })
      :UserKeys(VK_F7    , {|ob| DoEvents(), _wPost("_Find"  , ob:cParentWnd, "_Find"  ), .F. })
      :UserKeys(VK_F5    , {|ob| DoEvents(), _wPost("_Print" , ob:cParentWnd, "_Print" ), .F. })
   ...
   IF lChange                              // изменение было
      ...
      FOR EACH oCol IN oBrw:aColumns
         ...
         IF oCol:Cargo:lTotal
            ...
            IF lDelete                       // Restore
               oCol:Cargo:nTotal += aSumm[ nCol ]
            ELSE                             // Delete
               oCol:Cargo:nTotal -= aSumm[ nCol ]
            ENDIF
            // oCol:Cargo:nTotal := nSum - aSumm[nCol]
            //?? oCol:Cargo:lTotal, oCol:Cargo:nTotal, lDelete,"|"
...
FUNCTION ColumnEdit_STKB(oBrw,aVal)
   LOCAL cMsg, aTyp, aDim, cFld, cAls, nClmn, aCntMn, aIcon, a2Dim, aSpr, aRet, nTime
...
STATIC FUNCTION ColumnEdit_J(ob,ni,cRun,cNm,aVal,cStr)  // без параметров возврата
   LOCAL cBlock, cMsg, xv, aRet, nTime, cAls
...
STATIC FUNCTION ColumnEdit_bPost(ob,ni,cRun,cNm,aVal,cStr,oc)  // без параметров возврата
   LOCAL cBlock, cMsg, aRet, lModify, xGet, nTime, cAls
...
FUNCTION myTsbEnd( oBrw )
   LOCAL cMaska, cMaska2, nLen, cAnsOem, cDat, lBottom, cTtl, cEnd
