...
#include "tsbrowse.ch"
*-----------------------------------------------------------------------------*
FUNCTION Report_1(oWnd,oBrw)
*-----------------------------------------------------------------------------*
   LOCAL a2Dim, aList, aScope, aMsg, nI, aRpt, cDat, aLine, lDbf, cStr, oCol
   ...
   FOR nI := 1 TO ORDKEYCOUNT()  // общее кол-во записей по индексу
      ORDKEYGOTO(nI)
      cStr := ALLTRIM( (cMAls)->DOCUM )
      AADD( a2Dim, { cStr, UPPER((cMAls)->DOCUM), DTOS((cMAls)->DATEVVOD2) } )
   NEXT
   ...
   oBrw:nCLR_Gray  := CLR_HGRAY
   oBrw:nCLR_HGray := CLR_HGRAY
   oBrw:nCLR_Lines := CLR_GRAY

   oBrw:Enabled(.F.)  // блокировать область таблицы (строки серого цвета)
   ...
   FOR nI := 1 TO LEN(aList)
      aMsg := {nI, aList[nI], 0, 0.00, CTOD(""), TIME() }
      cDat := aScope[nI]
   ...
   IF lDbf
      ...
      IF LEN(aRet) > 0
         nI    := aRet[1]
         aLine := { aRet[2], aScope[nI] }
         ? SPACE(5) + "aLine=", HB_ValToExp(aLine)
      ENDIF
      (cAls)->( DbCloseArea() )
   ENDIF

   DbSelectArea(cMAls)
   oBrw:Enabled(.T.)    // разблокировать область таблицы (строки отображаются)
...
*-----------------------------------------------------------------------------*
   ...
   LOCAL oCel, oBrw, oGet, aReturn, hFont, aFont, aHW, aItogo
   LOCAL cMsg, aFldSum := {"F3", "F4"}  // поля dbf для расчёта итого
   ...
   DEFINE WINDOW Report AT nY,nX WIDTH nW HEIGHT nH TITLE cTitle ;
      ...
      owc:nHIco    := 55               // высота иконки на кнопке
      ...
      DEFINE TBROWSE oRpt OBJ oRpt AT nY, nX ALIAS cAls WIDTH nW-nG*2 HEIGHT nH-nG CELL ;
         FONT    oBrw:Cargo:aFont         ; // все фонты для таблицы  из пред.таблицы
         BRUSH   oBrw:Cargo:aClrBrush     ; // цвет фона под таблицей из пред.таблицы
...
STATIC FUNCTION myTsbColor(oRpt,oBrw)  // цвета изменить
   ...
   NEXT
   //oRpt:hBrush := CreateSolidBrush(aBClr[1], aBClr[2], aBClr[3])  // цвет фона под таблицей -- НЕ НАДО !!!
...
STATIC FUNCTION myTopMenu( nY, nX, nG, nHIco, owc )
   ...
   nHAlign := DT_RIGHT  //DT_LEFT   // горизонтальные кнопки: 0-DT_LEFT, 1-DT_CENTER, 2-DT_RIGHT
   aBtnObj := {}
   nHeight := nWTxt := 0
          //     1           2         3         4
   aCapt := { "Выбор"  , "Печать", "Эксель" , "Выход"  }
   aCEng := { "Choice" , "Print" , "Excel"  , "Esc"    }
   aCapt := IIF( App.Cargo:cLang == "RU", aCapt, aCEng )
   aPost := { "_2Enter", "_2Prn" , "_2Excel", "_2Exit" }
