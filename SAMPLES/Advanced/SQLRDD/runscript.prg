...
PROCEDURE Main(cScriptFile, cLogFile, nToCommit)
   ...
   DO WHILE !Empty(cSql := ProxStmt(h1))
      ...
      ? "Processing " + hb_ntos(nIssued) + " commands executed with " + hb_ntos(nErrors) + " error" + iif(nErrors != 1, "s", "")

   ENDDO

   oSql:Commit()

   fwrite(h2, CRLF + CRLF + hb_ntos(nIssued) + " command" + iif(nIssued > 1, "s", "") + " executed" + CRLF + hb_ntos(nErrors) + " error" + iif(nErrors != 1, "s", "") + CRLF + CRLF)
