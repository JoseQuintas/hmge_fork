...
PROCEDURE Main( cProjArg )
  ...
  lVistaOrSe7en := ( IsVista() .OR. IsSeven() .OR. '10' $ WindowsVersion() [1] )
  ...
  IF DateTest( lRebuild, cProjFolder, cMainBase + cMainExt, aSeaFiles )
    ...
    ELSE
      ...
      ELSE
        ...
        cOut += 'echo ' + AddQuote( cHRBLIB + 'xhb.lib' )      + ' + >> ' + cRsp + CRLF
      ENDIF
      FOR nFile := 1 TO nLibFiles
        cFile := AllTrim( Mpm.LibList.Item( nFile ) )
      ...
      IF lGui
        ...
        cOut += 'echo ' + AddQuote( cHRBLIB + 'bostaurus.lib' )  + ' + >> ' + cRsp + CRLF
      ENDIF
      IF Mpm.ZipLib.Value
        cOut += 'echo ' + AddQuote( cHRBLIB + 'hbziparc.lib' )   + ' + >> ' + cRsp + CRLF
        cOut += 'echo ' + AddQuote( cHRBLIB + 'hbmzip.lib' )     + ' + >> ' + cRsp + CRLF
        cOut += 'echo ' + AddQuote( cHRBLIB + 'minizip.lib' )    + ' + >> ' + cRsp + CRLF
        cOut += 'echo ' + AddQuote( cHRBLIB + 'hbzlib.lib' )     + ' + >> ' + cRsp + CRLF
      ENDIF
...
LOCAL nBorder := GetBorderWidth()
  ...
  Environment.Ok.Row     := Max( 70, nEnvirHeight - 60 - iif( lThemed, 10, 0 ) )
  Environment.Cancel.Col := Max( 10, Int( nEnvirWidth / 2 ) + 10 )
  Environment.Cancel.Row := Max( 70, nEnvirHeight - 60 - iif( lThemed, 10, 0 ) )
...
PROCEDURE ShowAssociation( cExt )
  ...
  BEGIN SEQUENCE
    ...
    IF !IsRegistryKey( HKEY_CURRENT_USER, cKey )
      BREAK
    ENDIF
    cVal := GetRegistryValue( HKEY_CURRENT_USER, cKey )
    IF Empty( cVal )
      BREAK
    ENDIF
...
PROCEDURE ChangeAssociation( cExt, lSet )
  ...
  CASE lSet .AND. !lAssoc
    BEGIN SEQUENCE
      ...
      IF !CreateRegistryKey( HKEY_CURRENT_USER, cKey )
        BREAK
      ENDIF
      IF !SetRegistryValue( HKEY_CURRENT_USER, cKey,, 'mpmfile' )
        BREAK
      ENDIF
      ...
      IF !CreateRegistryKey( HKEY_CURRENT_USER, cKey )
        BREAK
      ENDIF
      cKey2 := 'Software\CLASSES\mpmfile\mpmfile\DefaultIcon'
      IF !CreateRegistryKey( HKEY_CURRENT_USER, cKey2 )
        BREAK
      ENDIF
      IF !SetRegistryValue( HKEY_CURRENT_USER, cKey2,, ;
        AddQuote( GETEXEFILENAME() ) + ',-1' )
        BREAK
      ENDIF
      IF !SetRegistryValue( HKEY_CURRENT_USER, 'Software\CLASSES\mpmfile\shell',, 'open' )
        BREAK
      ENDIF
      IF !SetRegistryValue( HKEY_CURRENT_USER, cKey,, ;
        AddQuote( GETEXEFILENAME() ) + ' "%1"' )
        BREAK
      ENDIF
    RECOVER
  ...
  CASE !lSet .AND. lAssoc
    BEGIN SEQUENCE
      cKey := 'Software\CLASSES\' + cExt
      IF !DeleteRegistryKey( HKEY_CURRENT_USER, cKey )
        BREAK
      ENDIF
    RECOVER
...
PROCEDURE LoadEnvironment
  ...
  LOCAL cIniFolder  := AddSlash( GetApplicationDataFolder() + "\MPM" )
  LOCAL cIniFile    := cIniFolder + 'mpm.ini'
  ...
  IF ! hb_DirExists( cIniFolder )
     hb_DirBuild( cIniFolder )
  ENDIF
...
PROCEDURE SaveEnvironment
  ...
  SaveCheck()
  MemoWrite( cIniFolder + 'mpm.ini', cEnvir )
...
FUNCTION HelpText
  ...
  LOCAL cText := ;
    ...
    "\par" + CRLF + ;
    "\b BCC folder\b0 :  Location of BCC, the folder cotaining its Bin, Include, and Lib folders.  Defaults to \b C:\\Borland\\BCC58\b0 .\par" + CRLF + ;
    "\par" + CRLF + ;
    ...
    "{\pntext\f2\'B7\tab}hbwin.lib (Harbour only)\par" + CRLF + ;
