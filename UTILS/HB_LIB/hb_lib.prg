 ...
 * Author: Igor Nazarov
 *
 * Revised by Grigory Filatov <gfilatov@gmail.com>
 */
...
/****************************************************************************/
   ...
   lIniFile := hb_FileExists( cIniFile )

   // Read paths from INI file
   BEGIN INI FILENAME ( cIniFile )
      GET cTLibPath SECTION 'Path' ENTRY 'TLib' DEFAULT "C:\BORLAND\BCC58\BIN"
   ...
   DEFINE FONT FontBold FONTNAME _HMG_DefaultFontName ;
      SIZE _HMG_DefaultFontSize + 4 BOLD
   hFont := GetFontHandle( "FontBold" )
   ...
   DEFINE WINDOW Form_0 ;
      ...
      DEFINE BUTTONEX Button_Export
         ROW 520
         COL 210
         WIDTH 110
         HEIGHT 26
         ACTION ( BRW_1:ExcelOle( "Export.xls", .T.,, { "Content of libraries Exported", hFont } ), BRW_1:GoTop(), BRW_1:Refresh( .T. ) )
         CAPTION "Export to Excel"
         TOOLTIP 'Export Browse content to Excel'
      END BUTTONEX
   ...
   BRW_1:bLogicLen := {|| iif( Empty( ( BRW_1:cAlias )->( dbFilter() ) ), ;
      ( BRW_1:cAlias )->( LastRec() ), ;
      ( nLen := 0, ( BRW_1:cAlias )->( dbEval( {|| nLen++ }, &("{||" + ( BRW_1:cAlias )->( dbFilter() ) + "}" ) ) ), nLen ) ) }
   FILTER_LIB()
...
/****************************************************************************/
   ...
   IF File( cTLibPath + '\tlib.exe' ) // If found the file TLib.exe
      ...
      Gauge.Hide()
   ELSE
      MsgBox( "Cannot found the required EXE module " + CRLF + cTLibPath + '\tlib.exe ' )
   END

RETURN NIL

/****************************************************************************/
FUNCTION FILTER_LIB() // Database filter
/****************************************************************************/
   LOCAL cFilter := ""
   LOCAL bFilter
   ...
   IF Form_0.Check_lib.Value
      cFilter += iif( ! Empty( cFilter ), " .AND. ", "" ) + "LIBRARY = '" + Form_0.Combo_lib.DisplayValue + "'"
   END
